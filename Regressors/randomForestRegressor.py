...
# MSE (RF): 5228536693112.166 , talvez cortar valores acima de um certo valor porque esta em segundos
...
print("Training time (RF):", end - start)
...
plt.title("Random Forest Regression - Response Time")
plt.show()
...
end_pca = time.time()
print("Training time (RF + PCA):", end_pca - start_pca)

y_pred_pca = rf_reg_pca.predict(X_test_pca)
print("MSE (RF + PCA):", mean_squared_error(y_test_pca, y_pred_pca))
print("R2 (RF + PCA):", r2_score(y_test_pca, y_pred_pca))

cv_scores_pca = cross_val_score(rf_reg_pca, X_pca, Y, cv=5, scoring='r2')
...
plt.title("Random Forest Regression with PCA - Response Time")
plt.show()
