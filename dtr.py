...
# MSE (Decision Tree): 9407876878134.129, talvez cortar valores acima de um certo valor porque esta em segundos
...
print("Training time (Decision Tree):", end - start)

y_pred = dt_reg.predict(X_test)
print("MSE (Decision Tree):", mean_squared_error(y_test, y_pred))
print("R2 (Decision Tree):", r2_score(y_test, y_pred))

cv_scores = cross_val_score(dt_reg, X_scaled, Y, cv=5, scoring='r2')
...
plt.title("Decision Tree Regression - Response Time")
plt.show()

plt.figure(figsize=(18, 10))
plot_tree(dt_reg, feature_names=X.columns, filled=True, max_depth=3)
plt.title("Decision Tree Regression (Partial Tree View)")
plt.show()
...
dt_reg_pca = DecisionTreeRegressor(random_state=42)

start_pca = time.time()
dt_reg_pca.fit(X_train_pca, y_train_pca)
end_pca = time.time()
print("Training time (Decision Tree + PCA):", end_pca - start_pca)

y_pred_pca = dt_reg_pca.predict(X_test_pca)
print("MSE (Decision Tree + PCA):", mean_squared_error(y_test_pca, y_pred_pca))
print("R2 (Decision Tree + PCA):", r2_score(y_test_pca, y_pred_pca))

cv_scores_pca = cross_val_score(dt_reg_pca, X_pca, Y, cv=5, scoring='r2')
print("Mean R2 (cross-validation, PCA):", cv_scores_pca.mean())
print("Standard deviation (PCA):", cv_scores_pca.std())

plt.scatter(y_test_pca, y_pred_pca, alpha=0.6, color='darkred')
plt.xlabel("Actual response time")
plt.ylabel("Predicted response time (PCA)")
plt.title("Decision Tree Regression with PCA - Response Time")
plt.show()
