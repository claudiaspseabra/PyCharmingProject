...
print("Training time (GBR):", end - start)

y_pred = gb_reg.predict(X_test)
print("MSE (GBR):", mean_squared_error(y_test, y_pred))
print("R2 (GBR):", r2_score(y_test, y_pred))

cv_scores = cross_val_score(gb_reg, X_scaled, Y, cv=5, scoring='r2')
...
plt.title("Gradient Boosting Regression - Response Time")
plt.show()

# Gradient Boosting com PCA
...
gb_reg_pca = GradientBoostingRegressor(n_estimators=100, learning_rate=0.1, max_depth=3, random_state=42)

start_pca = time.time()
gb_reg_pca.fit(X_train_pca, y_train_pca)
end_pca = time.time()
print("Training time (GBR + PCA):", end_pca - start_pca)

y_pred_pca = gb_reg_pca.predict(X_test_pca)
print("MSE (GBR + PCA):", mean_squared_error(y_test_pca, y_pred_pca))
print("R2 (GBR + PCA):", r2_score(y_test_pca, y_pred_pca))

cv_scores_pca = cross_val_score(gb_reg_pca, X_pca, Y, cv=5, scoring='r2')
print("Mean R2 (cross-validation, PCA):", cv_scores_pca.mean())
print("Standard deviation (PCA):", cv_scores_pca.std())

plt.scatter(y_test_pca, y_pred_pca, alpha=0.6, color='orange')
plt.xlabel("Actual response time")
plt.ylabel("Predicted response time (PCA)")
plt.title("Gradient Boosting Regression with PCA - Response Time")
plt.show()
