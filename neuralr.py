...
# MLP Regressor
mlp_reg = MLPRegressor(hidden_layer_sizes=(100, 50), max_iter=300, random_state=42)

start = time.time()
mlp_reg.fit(X_train, y_train)
end = time.time()

print("Training time (MLP Regressor):", end - start)

y_pred = mlp_reg.predict(X_test)
print("MSE (MLP Regressor):", mean_squared_error(y_test, y_pred))
print("R2 (MLP Regressor):", r2_score(y_test, y_pred))

cv_scores = cross_val_score(mlp_reg, X_scaled, Y, cv=5, scoring='r2')
print("Mean R2 (cross-validation, MLP Regressor):", cv_scores.mean())
print("Standard deviation (MLP Regressor):", cv_scores.std())

plt.scatter(y_test, y_pred, alpha=0.6, color='blue')
plt.xlabel("Actual response time")
plt.ylabel("Predicted response time")
plt.title("MLP Regression - Response Time Prediction")
plt.show()
...
mlp_reg_pca = MLPRegressor(hidden_layer_sizes=(100, 50), max_iter=300, random_state=42)

start_pca = time.time()
mlp_reg_pca.fit(X_train_pca, y_train_pca)
end_pca = time.time()

y_pred_pca = mlp_reg_pca.predict(X_test_pca)
print("Training time (MLP Regressor + PCA):", end_pca - start_pca)
print("MSE (MLP Regressor + PCA):", mean_squared_error(y_test_pca, y_pred_pca))
print("R2 (MLP Regressor + PCA):", r2_score(y_test_pca, y_pred_pca))

cv_scores_pca = cross_val_score(mlp_reg_pca, X_pca, Y, cv=5, scoring='r2')
print("Mean R2 (cross-validation, MLP Regressor + PCA):", cv_scores_pca.mean())
print("Standard deviation (MLP Regressor + PCA):", cv_scores_pca.std())

plt.scatter(y_test_pca, y_pred_pca, alpha=0.6, color='green')
plt.xlabel("Actual response time")
plt.ylabel("Predicted response time (PCA)")
plt.title("MLP Regression with PCA - Response Time Prediction")
plt.show()
